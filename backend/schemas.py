...
class LeadCreate(BaseModel):
    ...
    model_config = ConfigDict(extra="ignore")
    # Optional as of the re-sequenced quiz funnel: MasterQuiz now creates
    # the lead right after the result screen, before city is ever asked —
    # city arrives later via POST /leads/{id}/clinic-recommendation-preference.
    # Other lead-capture forms (OrthodonticsQuiz, TreatmentQuiz, etc.) still
    # send it upfront; both are valid.
...
class ClinicRecommendationPreferenceBody(BaseModel):
    """Step 3 of the re-sequenced quiz funnel — asked only after contact
    details are already unlocked. `wants_recommendations=False` ends the
    flow with no city ever collected; `True` requires city_slug and
    (re)runs clinic auto-matching now that a city is finally known."""
    model_config = ConfigDict(extra="ignore")
    wants_recommendations: bool
    city_slug: Optional[str] = Field(default=None, min_length=1, max_length=50)
    district_slug: Optional[str] = Field(default=None, max_length=50)
    # Optional "help the clinic prepare" fields — same shape MasterQuiz's
    # old `form` step used to send inside `answers`.
    importance: Optional[str] = Field(default=None, max_length=40)
    has_files: Optional[List[str]] = None
    preferred_channel: Optional[str] = Field(default=None, max_length=40)
    can_travel: Optional[str] = Field(default=None, max_length=10)
...
class Lead(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    # Optional — see LeadCreate.city_slug comment. May be set at creation
    # (older/other quiz forms) or later via clinic-recommendation-preference.
    ...
    # None = not asked yet / not answered. True = opted in (city_slug set
    # via the same request). False = explicitly declined — the flow ends
    # there, no city, no clinics. Set only by
    # POST /leads/{id}/clinic-recommendation-preference.
    wants_clinic_recommendations: Optional[bool] = None
    clinic_recommendations_declined_at: Optional[str] = None
