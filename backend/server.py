...
@api_router.post("/admin/clinic-applications/{app_id}/regenerate-password")
async def regenerate_clinic_password_by_app(app_id: str, request: Request, user: AdminUser = Depends(get_current_user)):
    """Regenerate password for a clinic found by its application_id"""
    ...
    return {
        "status": "ok",
        "credentials": {"email": clinic["email"], "password": new_password},
        "email_sent": email_sent,
    }
