...
// ─── 2.5 Lumi explainer video — sits between Hero and Trust strip ──
//
// Behavior contract (per product spec):
//   • preload="none" — the actual MP4 is NOT requested until the user
//     interacts. Only the lightweight WebP poster + a tiny <video>
//     shell ship with initial HTML.
//   • playsInline + muted=false default + controls — we honor the
//     user's intent: no autoplay, no surprise sound, native controls
//     once they choose to play.
//   • Section content (video DOM) only mounts when the section is
//     within ~200px of the viewport — IntersectionObserver gate. This
//     keeps the homepage initial paint identical to before.
//   • Mobile order: headline → video → bullets → CTA (per spec).
//
// Assets:
//   /videos/lumi-homepage-explainer.mp4 (720p web, ~900KB)
//   /images/lumi-homepage-poster.webp   (~72KB)
function LumiVideoSection() {
  const [videoActive, setVideoActive] = useState(false)
  const [mounted, setMounted] = useState(false)
  const wrapRef = useRef<HTMLDivElement | null>(null)
  const videoRef = useRef<HTMLVideoElement | null>(null)

  // Lazy-mount the <video> element only when the user scrolls near the
  // section. Until then we render the poster as a plain <img>, which
  ...
  function handlePlay() {
    setVideoActive(true)
    // Give React a tick to render the <video>, then trigger playback.
    setTimeout(() => {
      const v = videoRef.current
      if (v) {
        // We intentionally do NOT mute — user is opting in to play.
  ...
  return (
    ...
    >
      {/* Warm ivory backdrop with soft turquoise wash — matches the
          rest of the patient layer (FCFAF8 base + teal radial). */}
      <div
        aria-hidden
        className="absolute inset-0 pointer-events-none"
        style={{
      ...
      <div className="relative max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          {/* Headline + sub — appears first on mobile (order-1), left col on desktop (md:order-1). */}
          <Reveal>
            <div className="order-1 md:order-1">
              ...
              {/* On mobile we hide the bullets+CTA here and re-render below the
                  video (order-3) to satisfy the spec ordering. On desktop the
                  whole left column stays together. */}
          ...
          {/* Video card — right col on desktop (order-2), middle on mobile (order-2). */}
          <Reveal delay={120}>
              ...
              <div className="relative mx-auto max-w-sm md:max-w-md">
                ...
                >
                  {!videoActive && (
                    ...
                    >
                      {/* Poster — eagerly rendered, cheap, branded fallback bg behind it */}
                      <img
                        ...
                        decoding="async"
                      />
                      {/* Soft top-to-bottom darken for play affordance */}
          ...
          {/* Mobile-only bullets + CTA (order-3 on mobile). Hidden on desktop
              because the left column already has them. */}
