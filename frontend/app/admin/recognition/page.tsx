...
import { CheckCircle2, Heart, Loader2, Trash2, XCircle } from 'lucide-react'
...
export default function AdminRecognitionPage() {
  ...
  const updateClinicTag = async (entryId: string, clinicId: string | null) => {
    setActing(entryId)
    try {
      const response = await fetch(`${API_URL}/api/admin/recognition/${entryId}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteEntry = async (entryId: string) => {
    if (!window.confirm('Изтрий тази история завинаги? Действието не може да бъде отменено.')) return
    setActing(entryId)
    try {
      const response = await fetch(`${API_URL}/api/admin/recognition/${entryId}`, {
        method: 'DELETE',
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      {loading ? <div className="grid min-h-72 place-items-center"><Loader2 className="h-7 w-7 animate-spin text-teal-600" /></div> : entries.length === 0 ? <div className="mt-7 rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">Няма истории в този статус.</div> : <div className="mt-7 space-y-4">{entries.map((entry) => <article key={entry.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="font-semibold text-slate-950">{entry.display_name_public || 'Анонимен пациент'}</p><p className="mt-1 text-xs text-slate-500">{entry.treatment_label || 'Без посочена тема'} · {new Date(entry.created_at).toLocaleDateString('bg-BG')}</p></div>
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{entry.photo_layout}</span>
            <button
              onClick={() => deleteEntry(entry.id)}
              disabled={acting === entry.id}
              title="Изтрий завинаги"
              className="inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-white px-3 py-1 text-xs font-medium text-rose-700 hover:bg-rose-50 disabled:opacity-50"
            >
        ...
        <div className="mt-3 max-w-sm">
          <p className="mb-1.5 text-xs font-medium text-slate-500">Клиника (по избор)</p>
          <ClinicPicker
            value={entry.clinic_id ?? null}
            onChange={(clinicId) => updateClinicTag(entry.id, clinicId)}
            placeholder="Свържи с клиника…"
            fallbackName={entry.clinic_name}
          />
        </div>
