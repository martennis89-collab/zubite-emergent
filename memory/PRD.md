...
- **Premium Branded Review Poster (May 2026)** — Upgraded the printable review poster from a basic utility output to a Care-Pass-aligned brand asset. New reusable component `/app/frontend/components/ReviewPoster.tsx` renders both an on-screen scaled preview (`variant="preview"`) and the exact A4-portrait print version (`variant="print"`). Design: warm-ivory base (`#FAFBFD`), soft sky/cyan gradient blobs, subtle dotted SVG pattern, serif Zubite.bg wordmark with sky-blue `.bg` accent, "Платформа за орална грижа" pill, eyebrow "Обратна връзка от пациенти", serif H1 "Споделете мнение за вашето посещение" with sky-blue accent on "мнение", dynamic clinic name + city, large QR (380px on print / 180px on preview) inside a rounded white card with soft sky shadow, monospace URL fallback under the QR, three-pill trust strip ("Около 60 секунди · Без регистрация · Преглежда се преди публикуване"), and a friendly footer line "Вашето мнение помага на други пациенти да направят по-информиран избор." Backend `/api/clinic/reviews/collection-link` now also returns `clinic_name` + `city_name` so the poster can be personalized. New "Преглед на постера" button on the clinic page lets clinics see exactly what they will print before clicking "Принтиране на постер". Print CSS sets `@page { size: A4 portrait; margin: 0 }` and exact-color rendering. No review submission/moderation logic touched. Backend pytest 22/22 PASS.
